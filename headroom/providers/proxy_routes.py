...
def _codex_usage_snapshot_response() -> JSONResponse:
    ...
    return JSONResponse(
        ...
        headers={"x-headroom-codex-usage-source": "snapshot" if snapshot is not None else "empty"},
    )
...
async def _handle_codex_usage(proxy: Any, request: Request) -> Response:
    ...
    logger.info(
        "Codex usage request using local fallback after upstream status %d", response.status_code
    )
    return fallback
